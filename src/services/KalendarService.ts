...
export default class KalendarService {
    static getData(): KalendarDataItem[] {
        const data = localStorage.getItem('kalendar-data')
    ...
    static addTask(): KalendarDataItem {
        ...
        localStorage.setItem('kalendar-data', JSON.stringify(data))
    ...
    static deleteTask(taskId: number): number | void {
        ...
        if (targetKalendarItemIndex !== -1) {
            ...
            localStorage.setItem('kalendar-data', JSON.stringify(updatedData))
        }
